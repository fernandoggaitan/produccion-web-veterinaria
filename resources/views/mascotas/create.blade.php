<x-base title="Mascota nueva">
    ...
    <x-h1> {{ __('pets.title_create') }} </x-h1>
    ...
    <form class="mx-auto" action="{{ route('mascotas.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-5 group">
            <div class="mb-5">
                ...
                <select name="categoria_id" id="categoria_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value=""> {{ __('pets.form_save.three') }} </option>
                    @foreach ($categorias as $categoria)
                        <option @selected( old('categoria_id') == $categoria->id) value="{{ $categoria->id }}"> {{ $categoria->nombre }} </option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="mb-5 group">
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="imagen"> {{ __('pets.form_save.five') }} </label>
        ...
        <div>
            <button type="submit" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"> {{ __('pets.btn_add_pet') }} </button>
